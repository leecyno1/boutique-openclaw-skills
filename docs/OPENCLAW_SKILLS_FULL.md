...
- Ready (eligible): `99`
- Not ready: `26`
- OpenClaw 内置默认 skills: `34`
- 非内置 skills: `91`

> Source: `openclaw skills list --json`

## 1) OpenClaw 内置默认 skills（单独分类，不附 GitHub 地址）

| Skill | Eligible | Disabled | Missing requirements |
|---|---:|---:|---|
| `1password` | no | yes | - |
| `bear-notes` | no | yes | - |
| `blucli` | no | yes | - |
| `bluebubbles` | no | yes | config: channels.bluebubbles |
| `camsnap` | no | yes | - |
| `clawhub` | yes | no | - |
| `coding-agent` | yes | no | - |
| `discord` | no | yes | config: channels.discord.token |
| `eightctl` | no | yes | - |
| `gemini` | no | yes | - |
| `gh-issues` | yes | no | - |
| `gifgrep` | no | yes | - |
| `gog` | no | yes | - |
| `goplaces` | no | yes | env: GOOGLE_PLACES_API_KEY |
| `healthcheck` | no | yes | - |
| `imsg` | no | yes | - |
| `mcporter` | yes | no | - |
| `nano-banana-pro` | no | no | env: GEMINI_API_KEY |
| `notion` | no | yes | env: NOTION_API_KEY |
| `obsidian` | no | yes | - |
| `openai-image-gen` | yes | no | - |
| `openai-whisper-api` | yes | no | - |
| `openhue` | no | yes | - |
| `ordercli` | no | yes | - |
| `sag` | no | yes | env: ELEVENLABS_API_KEY |
| `sherpa-onnx-tts` | yes | no | - |
| `slack` | no | yes | config: channels.slack |
| `songsee` | no | yes | - |
| `sonoscli` | no | yes | - |
| `spotify-player` | no | yes | - |
| `trello` | no | yes | env: TRELLO_API_KEY, TRELLO_TOKEN |
| `voice-call` | yes | no | - |
| `wacli` | no | yes | - |
| `xurl` | yes | no | - |


## 2) 非内置 skills（按仓库分类要求 + GitHub 地址）
...
| Skill | Eligible | Disabled | Source | GitHub |
|---|---:|---:|---|---|
| `apple-calendar` | yes | no | `openclaw-managed` | [repo](https://github.com/search?q=apple-calendar&type=repositories) |
| `apple-notes` | yes | no | `openclaw-workspace` | [repo](https://github.com/antoniorodr/memo) |
...
`1password`, `agent-browser`, `ai-image-generation`, `ai-music-generation`, `ai-music-prompts`, `apple-calendar`, `apple-notes`, `apple-reminders`, `archive`, `atxp`, `banner-creator`, `baoyu-article-illustrator`, `baoyu-comic`, `baoyu-cover-image`, `baoyu-infographic`, `baoyu-post-to-wechat`, `baoyu-post-to-x`, `baoyu-slide-deck`, `baoyu-url-to-markdown`, `baoyu-xhs-images`, `bear-notes`, `bird`, `blogwatcher`, `blucli`, `bluebubbles`, `brainstorming`, `brave-search`, `byterover`, `camsnap`, `capability-evolver`, `chrome-devtools-mcp`, `clawhub`, `coding-agent`, `content-strategy`, `cron-wake`, `database`, `discord`, `docx`, `domain-hunter`, `eightctl`, `elevenlabs-music`, `feishu-doc`, `feishu-drive`, `feishu-perm`, `feishu-wiki`, `finance-data`, `find-skills`, `format-pro`, `frontend-design`, `gemini`, `gh-issues`, `gifgrep`, `github`, `gog`, `goplaces`, `healthcheck`, `himalaya`, `imsg`, `infographic-pro`, `larry`, `logo-creator`, `marketing-psychology`, `mcp-builder`, `mcporter`, `minimax-image-understanding`, `minimax-understand-image`, `minimax-web-search`, `model-usage`, `nano-banana-pro`, `nano-pdf`, `nanobanana`, `news-radar`, `notion`, `obsidian`, `openai-image-gen`, `openai-whisper`, `openai-whisper-api`, `openhue`, `oracle`, `ordercli`, `pdf`, `peekaboo`, `pptx`, `prisma-database-setup`, `proactive-agent`, `producthunt`, `programmatic-seo`, `reddit`, `reflection`, `requesthunt`, `sag`, `search`, `self-improving-agent`, `seo-geo`, `session-logs`, `shell`, `sherpa-onnx-tts`, `skill-creator`, `skill-name`, `skill-security-auditor`, `slack`, `social-content`, `songsee`, `sonoscli`, `spotify-player`, `subagent`, `summarize`, `tailwind`, `tailwind-design-system`, `tavily-search`, `tdd`, `test-driven-development`, `things-mac`, `tmux`, `todo`, `todoist-api`, `trello`, `tts`, `twitter`, `video-frames`, `voice-call`, `wacli`, `weather`, `web-search`, `xurl`
